...
# File: ssh_plugin.py
#
# Purpose: This file defines SshPlugin implementation for CTF.
#
# Note: This file was created at the NASA Johnson Space Center.
# =========================================================================================

"""
@namespace core_plugins.ssh_plugin
...
class SshController():
    ...
    def run_command_persistent(self, command, cwd="", prefix=":"):
        ...
        """
        log.info("Remote persistent command: {} cwd: {}".format(command, cwd))
        ...
        # Run {command} in background; wait for a second; redirect the pid of the background process to {pid_file}
        # Necessary to wait a second for the background process to start and the shell to evaluate its pid ($!)
        cmd_str = "{command} & sleep 1; echo $! > {pid_file}".format(command=command, pid_file=pid_file)
        with self.connection.cd(cwd):
            with self.connection.prefix(prefix):
                try:
        ...
        # Wait for 2 seconds, so the pid_file can flush the buffer
        # Without the wait, the next 'cat {pid_file}'s output could be empty
        Global.time_manager.wait(2)

        # Save the pid of the background process
        cmd_res = None
        pid_cmd_str = 'cat {pid_file}'.format(pid_file=pid_file)
        try:
            cmd_res = self.connection.run(pid_cmd_str)
        except (invoke.exceptions.UnexpectedExit, invoke.exceptions.CommandTimedOut) as exception:
            result = exception.result
            log.error("Remote run command {} failed: {}".format(pid_cmd_str, exception))
        if cmd_res and cmd_res.exited == 0 and cmd_res.stdout:
            self.last_pid = cmd_res.stdout.rstrip()
        else:
            self.last_pid = None
            log.error("Unable to get PID of last command: {} !".format(cmd_str))

        # Clear the content of the file {pid_file}
        cmd_res = None
        clear_pid_cmd_str = '> {pid_file}'.format(pid_file=pid_file)
        try:
            cmd_res = self.connection.run(clear_pid_cmd_str)
        except (invoke.exceptions.UnexpectedExit, invoke.exceptions.CommandTimedOut) as exception:
            result = exception.result
            log.error("Remote run command {} failed: {}".format(clear_pid_cmd_str, exception))
        if cmd_res and cmd_res.exited != 0:
            log.error("Unable to clear pid_file: {}".format(cmd_res.stdout))
