...
#
# Purpose: This file defines ValidationPlugin implementation for CTF.
#
...
class ValidationPlugin(Plugin):
    ...
    def run_shell_command(command):
        ...
        """
        log.info("Run shell command : {}".format(command))
        ...
        try:
            ...
            process = Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                            text=True, cwd='/', shell=True, universal_newlines=True)
            # Although Popen is non-blocking, process.communicate() blocks the execution until the command completes
            # as it waits for stdout/err. Add '&' at the end doesn't help, if process.communicate() is invoked.
            if not run_in_background:
                stdout, stderr = process.communicate()
                return_code = process.returncode
                if return_code == 0 and not stderr:
                    result = True
                log.debug("The command return code: {} stdout: '{}' stderr: '{}'".format(return_code, stdout, stderr))
        except CalledProcessError as exception:
            log.error("Command failed with return code: {} stdout: '{}' stderr: '{}'".format(exception.returncode,
                                                                                             exception.stdout,
                                                                                             exception.stderr))
        return result
