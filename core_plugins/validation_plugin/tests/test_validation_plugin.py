...
#
# Purpose: This file contains test cases for unit testing of ValidationPlugin functions.
#
...
from core_plugins.validation_plugin.validation_plugin import ValidationPlugin
...
def test_validation_plugin_save_file_as_text(validation_plugin):
    ...
    assert validation_plugin.save_file_as_text('core_plugins/validation_plugin/tests/evs.bin', 'evs.txt', 'EVS', '')
...
def test_validation_plugin_interpret_event_log(validation_plugin):
    ...
    assert validation_plugin.interpret_event_log('core_plugins/validation_plugin/tests/evs.bin', 'evs.txt', '')
...
def test_validation_plugin_run_shell_command(validation_plugin,utils):
    utils.clear_log()
    assert validation_plugin.run_shell_command("ls")
    assert utils.has_log_level("DEBUG")
    assert utils.has_log_level("INFO")
    assert utils.has_log("Run shell command : ls")


def test_validation_plugin_run_shell_command_background(validation_plugin,utils):
    utils.clear_log()
    assert validation_plugin.run_shell_command("ls &")
    assert not utils.has_log_level("DEBUG")
    assert utils.has_log_level("INFO")
    assert utils.has_log("Run shell command : ls")


def test_validation_plugin_run_shell_command_exception(validation_plugin,utils):
    utils.clear_log()
    with patch('core_plugins.validation_plugin.validation_plugin.Popen') as mock_popen:
        mock_popen.side_effect = subprocess.CalledProcessError(returncode=-1,cmd='mock exception')
        assert not validation_plugin.run_shell_command("ls")
        assert utils.has_log_level("ERROR")
