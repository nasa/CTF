...
#
# File: test_userio_plugin.py
#
# Purpose: This file contains test cases for unit testing of UserIOPlugin functions.
#
...
def test_userio_waituserinput_enter_y(userio_instance):
    ...
    """
    with patch('core_plugins.userio_plugin.userio_plugin.input', return_value='y'):
...
def test_userio_process_command(userio_instance):
    ...
    data = {'prompt': '  '}
    with patch('core_plugins.userio_plugin.userio_plugin.input') as mock_input:
...
def test_userio_waituserinput_enter_yy(userio_instance):
    ...
    expect return False for any input other than 'y' or 'Y'
    """
    with patch('core_plugins.userio_plugin.userio_plugin.input', return_value='yy'):
        assert not userio_instance.waituserinput_command('wait for user input:')
