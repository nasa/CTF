...
#
# File: test_ccsds_primary_header.py
#
# Purpose: This file contains test cases for unit testing of ccsds_primary_header functions.
#
...
from core_plugins.ccsds_plugin.ccsds_primary_header import CcsdsPrimaryHeaderBase
...
def test_ccsdsprimaryheaderbase_set_packet_length(primary_header_base):
    ...
    assert primary_header_base.get_packet_length() == 100

    assert primary_header_base.set_packet_length(7890) is None
    assert primary_header_base.length == 7890
    assert primary_header_base.get_packet_length() == 7890
    ...
    primary_header_base.set_packet_length(65536)
    assert primary_header_base.length != 65536
    assert primary_header_base.get_packet_length() != 65536
...
def test_ccsdsprimaryheaderbase_get_segmentation_flags(primary_header_base):
    """
    Test CcsdsPrimaryHeaderBase class method: get_segmentation_flags
    Returns the segmentation_flags value derived from the header fields
    """
    assert primary_header_base.set_segmentation_flags(CcsdsPacketInterface.CFE_MSG_SEGFLG_CNT) is None
    assert primary_header_base.get_segmentation_flags() == CcsdsPacketInterface.CFE_MSG_SEGFLG_CNT

    assert primary_header_base.set_segmentation_flags(CcsdsPacketInterface.CFE_MSG_SEGFLG_FIRST) is None
    assert primary_header_base.get_segmentation_flags() == CcsdsPacketInterface.CFE_MSG_SEGFLG_FIRST

    assert primary_header_base.set_segmentation_flags(CcsdsPacketInterface.CFE_MSG_SEGFLG_LAST) is None
    assert primary_header_base.get_segmentation_flags() == CcsdsPacketInterface.CFE_MSG_SEGFLG_LAST

    assert primary_header_base.set_segmentation_flags(CcsdsPacketInterface.CFE_MSG_SEGFLG_UNSEG) is None
    assert primary_header_base.get_segmentation_flags() == CcsdsPacketInterface.CFE_MSG_SEGFLG_UNSEG

    # test ("segmentation_flags", ctypes.c_uint16, 2), overflow
    assert primary_header_base.set_segmentation_flags(5) is None
    assert primary_header_base.get_segmentation_flags() != 5