...
#
# File: test_ccdd_export_reader.py
#
# Purpose: This file contains test cases for unit testing of ccdd_export_reader functions.
#
...
from core_plugins.ccsds_plugin.readers.ccdd_export_reader import CCDDExportReader, ctypes_name, dynamic_init, \
...
def test_ccdd_export_reader_process_command_exception(ccdd_export_reader, utils):
    ...
    utils.clear_log()
    with patch('core_plugins.ccsds_plugin.readers.ccdd_export_reader.CCDDExportReader._create_parameterized_type') \
            as mock_create:
...
def test_ccdd_export_reader_create_parameterized_type_exception(ccdd_export_reader, utils):
    ...
    with patch('core_plugins.ccsds_plugin.readers.ccdd_export_reader.create_type_class') as mock_create:
...
def test_ccdd_export_reader_process_custom_types_errors(ccdd_export_reader):
    ...
    with patch('core_plugins.ccsds_plugin.readers.ccdd_export_reader.CCDDExportReader._build_data_type_and_field') as mock:
...
def test_ccdd_export_reader_process_ccsds_json_file_exception(ccdd_export_reader, utils):
    ...
    with patch('core_plugins.ccsds_plugin.readers.ccdd_export_reader.CCDDExportReader.process_command') as mock_command:
...
def test_ctypes_create_to_str():
    ...
    reload(core_plugins.ccsds_plugin.readers.ccdd_export_reader)
...
def test_get_sorted_file_order(ccdd_export_reader):
    """
    Test CCDDExportReader class method: get_sorted_file_order

    This test tests behavior given the following relationship b/w files:
        File A :
            - contains a reference to CFE_TIME_SysTime_t
            - contains a reference to CFE_MSG_CommandHeader_t

        File B:
            - contains the definition for CFE_TIME_SysTime_t

        File C:
            - contains the definition for CFE_MSG_CommandHeader_t, but uses a reference to CCSDS_PrimaryHeader_t
              to form that definition.

        File D:
            - contains the definition for CCSDS_PrimaryHeader_t

        File E: CMD file with no CC (ex: Virtual CMD file)
            - contains a reference to CFE_TIME_SysTime_t
            - Has no parameters

    """
    # Arrange
    file_A_dict =  {"tlm_mid_name": "CFE_ES_HK_TLM_MID", "tlm_description": "", "tlm_data_type": "CFE_ES_HousekeepingTlm_t",
                    "tlm_parameters": [ { "name": "LastUpdateTime", "description": "Time of last table update", "array_size": "0",
                                          "data_type": "CFE_TIME_SysTime_t", "bit_length": "0", "parameters": []},
                                          { "name": "CmdHeaderReference", "description": "", "array_size": "0",
                                           "data_type": "CFE_MSG_CommandHeader_t", "bit_length": "0", "parameters": []}]}

    file_B_dict = {"tlm_mid_name": "CFE_TBL_HK_TLM_MID", "tlm_description": "", "tlm_data_type": "CFE_TBL_HousekeepingTlm_t",
                  "tlm_parameters": [ { "name": "Payload", "description": "Telemetry payload", "array_size": "0",
                                       "data_type": "CFE_TBL_HousekeepingTlm_Payload_t", "bit_length": "0", "parameters":
                                       [{ "name": "LastUpdateTime", "description": "", "array_size": "0",
                                         "data_type": "CFE_TIME_SysTime_t", "bit_length": "0", "parameters":
                                         [{ "name": "Seconds", "description": "", "array_size": "0", "data_type": "uint32",
                                           "bit_length": "0", "parameters": [] }, { "name": "Subseconds", "description": "",
                                            "array_size": "0", "data_type": "uint32", "bit_length": "0", "parameters": []}]}]}]}

    file_C_dict = {"data_type": "CFE_MSG_CommandHeader_t", "parameters": [ { "name": "pheader", "description": "",
                        "array_size": "0", "data_type": "CCSDS_PrimaryHeader_t", "bit_length": "0", "parameters": [] }]}

    file_D_dict = { "data_type": "CFE_MSG_TelemetryHeader_t", "parameters": [{ "name": "pheader", "description": "",
                                "array_size": "0", "data_type": "CCSDS_PrimaryHeader_t", "bit_length": "0",
                                "parameters": [{ "name": "version_number", "description": "", "array_size": "0",
                                                "data_type": "uint16", "bit_length": "3", "parameters": []}]}]}

    file_E_dict = {"cmd_mid_name": "CP_INBOUND_VIRTUAL_CMD_MID", 
                   "cmd_description": "", 
                   "cmd_data_type": "CP_WrappedCmd_t"}
    
    config = CfsConfig('cfs')
    directory = config.ccsds_data_dir
    file_A = directory + '/file_A.json'
    file_B = directory + '/file_B.json'
    file_C = directory + '/file_C.json'
    file_D = directory + '/file_D.json'
    file_E = directory + '/file_E.json'

    files_to_dicts = {file_A:file_A_dict, file_B:file_B_dict,
                      file_C:file_C_dict, file_D:file_D_dict,
                      file_E:file_E_dict}

    files = list(files_to_dicts.keys())

    for file, dict in files_to_dicts.items():
        with open(file, 'w') as open_file:
            json.dump(dict, open_file)

    # Act
    sorted_files = ccdd_export_reader.get_sorted_file_order(files)
    ...
    # Assert (dependency files must be read before files that reference them)
    assert len(sorted_files) == len(files)
    assert sorted_files.index(file_B) < sorted_files.index(file_A)
    assert sorted_files.index(file_B) < sorted_files.index(file_E)
    assert sorted_files.index(file_D) < sorted_files.index(file_C) < sorted_files.index(file_A)

def test_get_sorted_file_order_circular_ref(ccdd_export_reader, utils):
    # Arrange
    file_A_dict =  {"name": "X2_CmdRespEntry", "array_size": 0, "description": "", "data_type": "X2_CmdRespEntry_t",
                    "parameters": [{ "name": "usTimestampSubsecs", "array_size": 0, "data_type": "X3_CmdRespEntry_t",
                                    "bit_length": 0}]}


    file_B_dict = {"name": "x3RespEntries", "array_size": 0, "description": "", "data_type": "X3_CmdRespEntry_t",
                   "parameters": [{ "name": "usTimestampSubsecs", "array_size": 0, "data_type": "X4_CmdRespEntry_t",
                                    "bit_length": 0}]}


    file_C_dict = {"name": "x4RespEntries", "array_size": 0, "description": "", "data_type": "X4_CmdRespEntry_t",
                   "parameters": [{ "name": "usTimestampSubsecs", "array_size": 0, "data_type": "X2_CmdRespEntry_t",
                                     "bit_length": 0}]}
    ...
    files_to_dicts = {file_A:file_A_dict, file_B:file_B_dict, file_C:file_C_dict}
