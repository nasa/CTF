...
#
# Purpose: This file defines VariablePlugin implementation for CTF.
#
# Note: This file was created at the NASA Johnson Space Center.
# =========================================================================================

"""
@namespace core_plugins.variable_plugin
...
from lib.ctf_utility import resolve_variable, resolve_macros, type_map
...
class VariablePlugin(Plugin):
    ...
    def set_user_variable_from_tlm(variable_name: str, mid: str, tlm_variable: str, target: str = None,
        ...
        if tlm_value is None:
            log.error("No matched telemetry attribute found for MID {}".format(resolved_mid))
            return False
    ...
    def check_user_defined_variable(variable_name: str, operator: str, value, variable_type: str = None) -> bool:
       ...
       """
        ...
        if variable_value is None:
            log.error("Variable {} is None ".format(variable_name))
            return False
        if op_func is None:
        ...
        if variable_type:
            if variable_type not in type_map:
                log.error("Cast type {} not supported".format(variable_type))
                return False
            try:
                value = type_map[variable_type](value)
                variable_value = type_map[variable_type](variable_value)
            except (ValueError, SyntaxError) as exception:
                log.error("Could not type cast variable:'{}' or value:'{}'".format(variable_value, value))
                log.error("Invoke exception {}".format(exception))
                return False
