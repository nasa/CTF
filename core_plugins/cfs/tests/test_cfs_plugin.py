...
#
# Purpose: This file contains test cases for unit testing of cfs_plugin functions.
#
...
def test_cfs_plugin_register_cfs_invalid(cfs_plugin):
    ...
    # config errors
    with patch('core_plugins.cfs.cfs_plugin.CfsConfig.get_error_count', return_value=1):
        assert not cfs_plugin.register_cfs('cfs')
        assert not cfs_plugin.targets
...
def test_cfs_plugin_shutdown(cfs_plugin):
    ...
    with patch('core_plugins.cfs.pycfs.cfs_controllers.CfsController.shutdown'):
