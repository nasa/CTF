...
#
# Purpose: This file contains test cases for unit testing of CFS controller functions.
#
...
import os
import shutil
from unittest.mock import Mock, patch, mock_open, call, ANY
import tempfile
...
from core_plugins.cfs.pycfs.cfs_controllers import CfsController, RemoteCfsController, merge_ccsds_dictionaries
...
def _remote_controller_instance_inited():
    ...
    with patch('core_plugins.ssh.ssh_plugin.SshController.init_connection', return_value=True):
        controller.initialize()
        controller.cfs_pid = 42
    return controller
...
def test_cfs_controller_initialize_pass(cfs_controller):
    ...
    assert cfs_controller.cfs.command.socket_if.cfs_socket.getsockname() == ('0.0.0.0', 5011)
...
def test_cfs_controller_initialize_fail(cfs_controller, utils):
    ...
    """
    # cfs init fails
    with patch('core_plugins.cfs.pycfs.cfs_controllers.LocalCfsInterface') as mock_localcfsinterface:
...
def test_cfs_controller_initialize_exception(cfs_controller, utils):
    ...
    create command interface; create telemetry interface; create local CFS interface
    """
    with patch('core_plugins.cfs.pycfs.cfs_controllers.import_ccsds_header_types') as mock_import:
    ...
    with patch("core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.start_cfs") as mock_start_cfs:
...
def test_cfs_controller_build_cfs(cfs_controller):
    ...
    """
    with patch('core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.build_cfs', return_value=True) as mock_build:
        assert cfs_controller.initialize()
...
def test_cfs_controller_start_cfs(cfs_controller_inited):
    ...
    """
    with patch("core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.start_cfs") as mock_start_cfs, \
            patch('core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.enable_output'):
...
def test_cfs_controller_start_cfs_restart(cfs_controller_inited):
    ...
    it calls CfsController instance's start_cfs function.
    """
    with patch("core_plugins.cfs.pycfs.cfs_controllers.LocalCfsInterface") as mock_cfs:
...
def test_cfs_controller_enable_cfs_output(cfs_controller):
    ...
    """
    with patch('core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.enable_output'), \
            patch('core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.build_cfs', return_value=True) as mock_build:
        assert cfs_controller.initialize()
...
def test_cfs_controller_send_cfs_command(cfs_controller, workspace):
    with patch('core_plugins.cfs.pycfs.cfs_controllers.LocalCfsInterface'):
        cfs_controller.initialize()
...
def test_cfs_controller_encode_ctypes_to_bytes_array(cfs_controller, workspace):
    ...
    from core_plugins.ccsds_plugin.readers.ccdd_export_reader import create_type_class
    with patch('core_plugins.cfs.pycfs.cfs_controllers.LocalCfsInterface'):
        cfs_controller.initialize()
    ...
    from core_plugins.ccsds_plugin.readers.seds_reader import create_type_class
    with patch('core_plugins.cfs.pycfs.cfs_controllers.LocalCfsInterface'):
        cfs_controller.initialize()
        arr16_type = (ctypes.c_uint16 * 4)
...
def test_cfs_controller_check_tlm_value_pass(cfs_controller_inited):
    ...
    with patch("core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.check_tlm_value", return_value=True):
...
def test_cfs_controller_get_tlm_value(cfs_controller_inited):
    ...
    with patch("core_plugins.cfs.pycfs.local_cfs_interface.LocalCfsInterface.get_tlm_value", return_value=True):
...
def test_cfs_controller_convert_check_tlm_args(cfs_controller_inited):
    ...
    # value is dictionary
    args = [{'variable': 'Payload.CommandErrorCounter', 'value': {'a': '#FALSE#', 'b': '#TRUE#'}, 'compare': '=='}]
    assert cfs_controller_inited.convert_check_tlm_args(args)[0]['value']['a'] == 0
    assert cfs_controller_inited.convert_check_tlm_args(args)[0]['value']['b'] == 1
...
def test_cfs_controller_shutdown_cfs(cfs_controller_inited):
    ...
    with patch('subprocess.run') as mock_system:
        mock_system.return_value = Mock(returncode=0)
        assert cfs_controller_inited.shutdown_cfs()
        mock_system.assert_any_call(['kill', '-9', '42'], capture_output=ANY, check=ANY, text=ANY)
        mock_system.assert_called_with('pkill -fe -9 "tail -f /dev/null"', capture_output=ANY, check=ANY, shell=True, text=ANY)
    assert not cfs_controller_inited.cfs


def test_cfs_controller_shutdown_cfs_fail_pid(cfs_controller_inited, utils):
    """
    Test CfsController class shutdown_cfs method: subprocess.run call returns an error code for one process
    Implementation of CFS plugin instructions shutdown_cfs. When CFS plugin instructions
    (shutdown_cfs) is executed, it calls CfsController instance's shutdown_cfs function.
    """
    # case: no cfs pid
    with patch('subprocess.run') as mock_system:
        ...
        assert utils.has_log_level("ERROR")
        mock_system.assert_called_once_with('pkill -fe -9 "tail -f /dev/null"', capture_output=ANY, check=ANY, shell=True, text=ANY)


def test_cfs_controller_shutdown_cfs_fail_ps(cfs_controller_inited, utils):
    """
    Test CfsController class shutdown_cfs method: subprocess.run call returns an error code
    for process that's not found. When CFS plugin instructions(shutdown_cfs) is executed,
    it calls CfsController instance's shutdown_cfs function.
    """
    # case: ps fails
    with patch('subprocess.run') as mock_system:
        mock_system.return_value = Mock(returncode=1)
        assert not cfs_controller_inited.shutdown_cfs()
        assert not cfs_controller_inited.cfs
        assert utils.has_log_level("ERROR")
        mock_system.assert_any_call(['ps', '-p', '42'], capture_output=ANY, check=ANY, text=ANY)
        mock_system.assert_called_with('pkill -fe -9 "tail -f /dev/null"', capture_output=ANY, check=ANY, shell=True, text=ANY)
...
def test_cfs_controller_shutdown_cfs_fail_kill(cfs_controller_inited, utils):
    """
    Test CfsController class shutdown_cfs method: subprocess.run call returns an error code
    on attempt to kill a process that is found. When CFS plugin instructions(shutdown_cfs)
    is executed, it calls CfsController instance's shutdown_cfs function.
    """
    # case: kill and pkill fail
    with patch('subprocess.run') as mock_system:
        # The first call should succeed, and the subsequent ones should fail (since the process was already terminated)
        mock_system.side_effect = [Mock(returncode=0), Mock(returncode=1), Mock(returncode=1)]
...
def test_cfs_controller_shutdown(cfs_controller_inited, utils):
    ...
    """
    with patch('core_plugins.cfs.pycfs.cfs_controllers.CfsController.shutdown_cfs') as mock_shutdown_cfs:
        # case: nominal
...
def test_remote_cfs_controller_initialize_fail(remote_controller, utils):
    ...
    """
    with patch('core_plugins.ssh.ssh_plugin.SshController.init_connection', return_value=False):
...
def test_remote_cfs_controller_initialize_pass(remote_controller):
    ...
    with patch('core_plugins.ssh.ssh_plugin.SshController.init_connection', return_value=True), \
            patch('core_plugins.cfs.pycfs.cfs_controllers.RemoteCfsInterface') as mock_remotecfsinterface:
        mock_remotecfsinterface.return_value.init_passed = True
        assert remote_controller.initialize()
        assert mock_remotecfsinterface.call_args.args[2].socket_if.cfs_socket.getsockname() == ('0.0.0.0', 5011)
...
def test_remote_cfs_controller_shutdown_cfs(remote_controller_inited, utils):
    ...
    """
    with patch('core_plugins.ssh.ssh_plugin.SshController.run_command', return_value=True) as mock_command, \
            patch('core_plugins.ssh.ssh_plugin.SshController.get_file', return_value=True) as mock_get:
        utils.clear_log()
...
def test_remote_cfs_controller_shutdown_cfs_fail(remote_controller_inited, utils):
    ...
    (shutdown_cfs) is executed, it calls RemoteCfsController instance's shutdown_cfs function.
    """
    with patch('core_plugins.ssh.ssh_plugin.SshController.run_command', return_value=False), \
            patch('core_plugins.ssh.ssh_plugin.SshController.get_file', return_value=True):
        utils.clear_log()
        remote_controller_inited.cfs.cfs_std_out_path = '/tmp/ssh_out.txt'
...
def test_remote_cfs_controller_shutdown_cfs_fail_get_file(remote_controller_inited, utils):
    ...
    (shutdown_cfs) is executed, it calls RemoteCfsController instance's shutdown_cfs function.
    """
    with patch('core_plugins.ssh.ssh_plugin.SshController.run_command', return_value=True) as mock_run_command, \
            patch('core_plugins.ssh.ssh_plugin.SshController.get_file', return_value=False):
        utils.clear_log()
        remote_controller_inited.cfs.cfs_std_out_path = '/tmp/ssh_out.txt'
        assert not remote_controller_inited.shutdown_cfs()
...
def test_remote_cfs_controller_shutdown_cfs_fail_no_pid(remote_controller_inited, utils):
    # case: no cfs pid
    with patch('core_plugins.ssh.ssh_plugin.SshController.run_command') as mock_run_command, \
            patch('core_plugins.ssh.ssh_plugin.SshController.get_file', return_value=True):
        utils.clear_log()
...
def test_remote_cfs_controller_archive_cfs_files_pass(remote_controller_inited):
    ...
    with patch('core_plugins.ssh.ssh_plugin.SshController.run_command', return_value=True), \
            patch('core_plugins.ssh.ssh_plugin.SshController.get_file', return_value=True), \
