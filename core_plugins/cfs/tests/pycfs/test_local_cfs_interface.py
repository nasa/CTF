...
#
# File: test_local_cfs_interface.py
#
# Purpose: This file contains test cases for unit testing of LocalCfsInterface functions.
#
...
def local_cfs_interface_build_error(cfs_config, mid_map, ccsdsv2):
    ...
    with patch('core_plugins.cfs.pycfs.output_app_interface.ToApi', name='mock'), \
         ...
         patch('core_plugins.cfs.pycfs.local_cfs_interface.Popen', side_effect=Exception('mock exception')):
        return LocalCfsInterface(cfs_config, mock_tlm, mock_cmd, mid_map, ccsdsv2)
...
def test_local_cfs_interface_get_start_string(localcfs, utils):
    with patch('core_plugins.cfs.pycfs.local_cfs_interface.find_executable') as mock_find:
...
def test_local_cfs_interface_build_cfs_pass(localcfs):
    with patch('builtins.open', new_callable=mock_open()) as mock_file, \
            patch('core_plugins.cfs.pycfs.local_cfs_interface.Popen') as mock_popen:
...
def test_local_cfs_interface_start_cfs_pass(localcfs):
    with patch('os.path.exists', return_value=True), \
         patch('core_plugins.cfs.pycfs.local_cfs_interface.run') as mock_run, \
         patch('core_plugins.cfs.pycfs.local_cfs_interface.Popen') as mock_popen:
