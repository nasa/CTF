...
#
# File: test_remote_cfs_interface.py
#
# Purpose: This file contains test cases for unit testing of lower-level RemoteCfsInterface functions.
#
...
from core_plugins.ssh.ssh_plugin import SshController
...
def test_remote_cfs_interface_build_cfs_pass(remotecfs, utils):
    with patch('os.path.exists', return_value=False):
        ...
        remotecfs.execution_controller.run_command.assert_called_once_with(
            'make distclean; make TARGET=lx1 install 2>&1 | tee /tmp/cfs_build_cfs_output.txt',
            cwd='/cfs/build/dir')
...
def test_remote_cfs_interface_start_cfs_pass(remotecfs):
    ...
    remotecfs.execution_controller.run_command_persistent.assert_called_once_with(
        ...
        cwd='/cfs/run/dir')
    assert remotecfs.execution_controller.run_command.mock_calls[0][1] == ('ps -p 42 > /dev/null 2>&1',)
    assert remotecfs.execution_controller.run_command.mock_calls[1][1] == ('ps aux | grep core-lx1',)
    assert remotecfs.execution_controller.run_command.call_count == 2
...
def test_remote_cfs_interface_start_cfs_fail(remotecfs, utils):
    ...
    remotecfs.execution_controller.run_command_persistent.assert_called_once_with(
        './core-lx1 args >> /tmp/cfs_cfs_stdout.txt',
        cwd='/cfs/run/dir')
    remotecfs.execution_controller.run_command.assert_called_once_with("ps aux | grep core-lx1")
