...
# File: cfs_config.py
#
# Purpose: This file defines the expected fields in the cFS config section for a base target.
#
...
class CfsConfig:
    ...
    def __init__(self, name):
        ...
        self.command_msg_time_source = None
    ...
    def load_config_data(self, section_name):
        ...
        if section_name in self.sections:
            ...
            self.cfs_run_dir = self.load_optional_field(section_name, "cfs_run_dir", Global.config.get,
                                                        validate_function=expand_path)
            ...
            self.cfs_exe = self.load_optional_field(section_name, "cfs_exe", Global.config.get, default_value="")

            # Only validate optional executable directory if provided
            if self.cfs_run_dir and self.cfs_exe:
                self.validation.validate_file(os.path.join(self.cfs_run_dir, self.cfs_exe))
            else:
                log.debug("cFS executable directory not provided, skipping file validation..")
        ...
        self.command_msg_time_source = self.load_optional_field(section_name, "command_msg_time_source",
                                                                Global.config.getint, 0,
                                                                self.validation.validate_int)

        self.transport_layer_protocol = self.load_optional_field(section_name, "transport_layer_protocol",
                                                                 Global.config.getint, 2, self.validation.validate_int)

        # CCSDS_header file ccsds_v1/v2.py doesn't support setting the timestamp attributes of command header
        ccsds_header_path = self.load_optional_field("ccsds", "CCSDS_header_path", Global.config.get, default_value="")
        header_file = ccsds_header_path.split('/')[-1]
        if header_file in ["ccsds_v1.py", "ccsds_v2.py"]:
            self.command_msg_time_source = 0
