...
#
# File: remote_cfs_interface.py
#
# Purpose: This file defines the lower-level interface to communicate with cFS remotely over SSH.
#
...
- Inherits Cfs Interface - extends some of its functionality specifically for SSH.
"""
...
class RemoteCfsInterface(LocalCfsInterface):
    ...
    def __init__(self, config, telemetry, command, mid_map, ccsds, execution):
        ...
        super().__init__(config, telemetry, command, mid_map, ccsds, None)
    ...
    def start_cfs(self, run_args):
        ...
        # Dump cFS related process info for debug purpose
        self.execution_controller.run_command("ps aux | grep {}".format(self.config.cfs_run_cmd))
