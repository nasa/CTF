...
#
# Purpose: This file defines CFS controller implementation for CTF.
#
...
from core_plugins.cfs.pycfs.cfs_interface_utility import resolve_cfs_macro
from core_plugins.ccsds_plugin.ccsds_packet_interface import import_ccsds_header_types
from core_plugins.ccsds_plugin.readers.ccdd_export_reader import CCDDExportReader
from core_plugins.cfs.pycfs.local_cfs_interface import LocalCfsInterface
...
class CfsController:
    ...
    def _init_cfs_interface(self):
        ...
        command = CommandInterface(socket_interface)
        telemetry = TlmListener(socket_interface)
        self.cfs = LocalCfsInterface(self.config, telemetry, command, self.mid_map, self.ccsds, self.macro_map)
    ...
    def start_cfs(self, run_args):
        ...
        self.cfs_pid = result.get("pid")
        if self.cfs_pid is None:
            log.error("Unable to get pid of process {}! CTF may be unable to stop CFS.".format(self.config.cfs_exe))
    ...
    def send_raw_cfs_command(self, mid: str, cc: str, buffer: str, header_args: dict = None,
        ...
        log.info("Sending CFS Command to target: {}, {}:{} with payload: {} "
                 "header_args: {}".format(self.config.name, mid, cc, buffer, header_args))
        ...
        try:
            ...
            if payload_length is not None and payload is not None:
                if payload_length < len(payload):
                    log.debug("Payload's actual size {} is more than the specified payload_length {}: the sending buff "
                              "will be truncated to the payload_length".format(len(payload), payload_length))
                    payload = payload[0:payload_length]
                elif payload_length > len(payload):
                    log.debug("Payload's actual size {} is less than the specified payload_length {}: additional "
    ...
    def encode_ctypes_to_bytes(self, args: ctypes.Structure) -> bytes:
        ...
        def handle_field(arg_val, field, byte_offset, bit_offset=0):
            ...
            # my_type will be a ctypes class for collections, or a string otherwise
            # type(ctypes.c_char) is _ctypes.PyCSimpleType, the type of all ctypes primitives
            my_type = field_type._type_
            if isinstance(my_type, type(ctypes.c_char)):
                # If field_type has its own internal type, ie an array, field_val should be a list.
                # Serialize its items individually according to this type, which may be any ctypes primitive.
                my_size = ctypes.sizeof(my_type)
    ...
    def resolve_macros(self, arg):
        ...
        """
        return resolve_cfs_macro(arg, self.macro_map)
            ...
            if isinstance(arg, dict):
                ...
                if isinstance(arg["value"], list):
                    ...
                    for key, value in arg["value"].items():
                        arg["value"][key] = self.resolve_macros(value)
                else:
    ...
    def check_event(self, app_name, event_id, event_str=None, is_regex=False, event_str_args=None):
        ...
        if result:
            ...
            if event_str:
                ...
                log.debug("No msg provided; any message for App {} and Event ID {} will be matched.".format(
                    app_name, event_id))
                result = self.cfs.check_tlm_value(self.cfs.evs_long_event_msg_mid, args, discard_old_packets=False)
    ...
    def shutdown_cfs(self):
        ...
        # try to kill outer command (xterm, sh) run by CTF by pid
        killed_outer_cmd = False
        if self.cfs_pid:
            result = subprocess.run(['ps', '-p', str(self.cfs_pid)], capture_output=True, check=False, text=True)
            if result.returncode == 0:
                log.info(result.stdout)

                # Kill the running process
                result = subprocess.run(['kill', '-9', str(self.cfs_pid)], capture_output=True, check=False, text=True)

                if result.returncode == 0:
                    killed_outer_cmd = True
                else:
                    killed_outer_cmd = False
        ...
        # Kill the running cFS process by name
        killed_cfs_process = False
        cmd = f'pkill -fe -9 "{self.config.cfs_run_cmd}"'
        result = subprocess.run(cmd, capture_output=True, check=False, shell=True, text=True)
        if result.returncode == 0:
            killed_cfs_process = True
        else:
            killed_cfs_process = False
            # the 'pkill' command does not provide any meaningful stderr, only a return code
            log.error("Failed to kill any {}. CFS may have already exited or still be running!"
                      .format(self.config.cfs_exe))

        status = killed_outer_cmd and killed_cfs_process
        return status
...
class RemoteCfsController(CfsController):
    ...
    def _init_cfs_interface(self):
        ...
        if not result:
            ...
            udp_socket_if = UDPSocketInterface(self.config.cfs_target_ip, self.config.tlm_udp_port,
                                               self.config.cmd_udp_port)
            command = CommandInterface(udp_socket_if)
            telemetry = TlmListener(udp_socket_if)
    ...
    def shutdown_cfs(self):
        ...
        if self.cfs_pid:
            ...
            log.error("Process id of {} is unknown! CFS may still be running.".format(self.config.cfs_exe))
            result = False
        ...
        if not os.path.exists(stdout_final_path):
            if not self.execution.get_file(self.cfs.cfs_std_out_path, stdout_final_path, {'delete': True}):
                log.error("Cannot move CFS stdout file to script log directory.")
                result = False
            else:
