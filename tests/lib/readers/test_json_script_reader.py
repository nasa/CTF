...
def test_json_script_reader_init(json_script_reader):
    ...
    assert json_script_reader.input_script_path == 'scripts/example_tests/test_ctf_all_instructions.json'
    assert json_script_reader.script.input_file_path == 'scripts/example_tests'
...
def test_json_script_reader_init_exception(utils):
    ...
    """
    utils.clear_log()
    input_script_path = 'scripts/example_tests/test_ctf_all_instructions_fail.json'
...
def test_json_script_reader_resolve_command_data(json_script_reader):
    ...
    params = {'expectedCmdCnt': 1, 'expectedErrCnt': 0, 'destIp': '127.0.0.1', 'destPort': 5011, 'routeMask': 0,
              'fileDesc': 0}
    assert json_script_reader.resolve_command_data(data, params) == params
